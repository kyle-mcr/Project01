        ...
        if form.is_valid():
            ...
            for i in entries:
                if search in entries:
                    ...
                    return render(request, "wiki.html", {"entry": page_converted, "title": search, "form":SearchEntry()})
                if search.lower() in i.lower():
                    searched.append(i)
            return render(request, "search.html", {"searched": searched, "form":SearchEntry()})
...
def edit(request, title):
    if request.method == 'GET':
        ...
        return render(request, "edit.html", {"form": SearchEntry(), "edit":EditEntry(initial={'textarea': page}), 'title':title})
    else:
        ...
        if form.is_valid():
            ...
            util.save_entry(title,textarea)
            entry = markdowner.convert(util.get_entry(title))
            return render(request, "wiki.html", {"form": SearchEntry(), "entry": entry, "title": title})